...
class Visualiser: 
    def __init__(self, args: Args, model: Net, optimiser: Optimiser, device: Device, log, use_transforms: bool=True):
        self.model = model 
    ...
    def activation_maximise(self, image: Tensor) -> Tensor:
        ...
        self.model.eval()
        self.model(image)
        self.activations[self.layer] = self.activations[self.layer].requires_grad_().to(self.device)
    ...
    def visualise(self, image: Tensor) -> None:
        ...
        print(f"Visualising Layer {self.layer} in model {model_name} @ channel {self.channel_idx}")
